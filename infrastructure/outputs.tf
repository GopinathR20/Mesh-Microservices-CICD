...
# Optional Cosmos DB Outputs (with index fix)
...
output "cosmos_db_primary_key" {
  ...
  value       = length(azurerm_cosmosdb_account.cosmos) > 0 ? azurerm_cosmosdb_account.cosmos[0].primary_key : "Cosmos DB not created"
  sensitive   = true
}
